...
import type { LeadsStatsResponse } from "@/lib/kpiStatsApi";
import { KPI_QUERY_STALE_MS, KPI_REFETCH_INTERVAL_MS, refetchIntervalWhenVisible } from "./kpiQueryDefaults";

const SESSION_PREFIX = "sparkai:leads:stats:";

function readSession(baseId: number): { data: LeadsStatsResponse; updatedAt: number } | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.sessionStorage.getItem(`${SESSION_PREFIX}${baseId}`);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { data?: LeadsStatsResponse; timestamp?: number };
...
function writeSession(baseId: number, data: LeadsStatsResponse) {
...
export function useLeadsStatsQuery(baseId: number | null) {
  return useQuery({
    queryKey: ["leads-stats", baseId] as const,
    enabled: Boolean(baseId),
    staleTime: KPI_QUERY_STALE_MS,
    refetchInterval: refetchIntervalWhenVisible(KPI_REFETCH_INTERVAL_MS),
    ...seed(baseId),
    queryFn: async () => {
      const data = (await apiRequest(`/leads/stats?base_id=${baseId}`)) as LeadsStatsResponse;
